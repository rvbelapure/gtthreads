                                    ...
                                    struct timeval *timeslice)
{
	...
	uthread_pid pid = cfs_kthread->k_ctx->current_uthread->pid;
	if(sched_switch == 'p' && processes[pid].is_greedy)
		timeslice_us /= (2 * processes[pid].thread_count);
...
static void cfs_update_vruntime(cfs_uthread_t *cfs_uthread)
{
	...
	if(sched_switch == 'p')
		cfs_uthread->vruntime += (cputime_us * cfs_uthread->priority * processes[cfs_uthread->uthread->pid].thread_count);
	else
...
uthread_t *cfs_preemt_current_uthread(kthread_t *k_ctx)
{
	...
	if (cur_uthread->state == UTHREAD_DONE) {
		...
		for(int i = 0 ; i < processes[cur_uthread->pid].thread_count ; i++)
		{
			if((processes[cur_uthread->pid]).tids[i] == cur_uthread->tid)
			{
				(processes[cur_uthread->pid]).is_thread_done[i] = 1;
				break;
			}
		}
		int is_complete = 1;
		for(int i = 0 ; i < processes[cur_uthread->pid].thread_count ; i++)
		{
			if((processes[cur_uthread->pid]).is_thread_done[i] == 0)
			{
				is_complete = 0;
				break;
			}
		...
		{
			stop_all_threads = 1;
			char c = processes[cur_uthread->pid].is_greedy ? 'y' : 'n';
			printf("Process %d completed - is_greedy ? : %c\n",cur_uthread->pid,c);
		}
