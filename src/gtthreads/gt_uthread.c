...
int uthread_create(uthread_pid u_pid, uthread_tid *u_tid, uthread_attr_t *attr,
                   int(*start_routine)(void *), void *arg)
{
	...
	new_uthread->pid = u_pid;
	processes[u_pid].tids[(processes[u_pid].thread_count)] = new_uthread->tid;
	processes[u_pid].threads[(processes[u_pid].thread_count)] = new_uthread;
	processes[u_pid].thread_count++;
