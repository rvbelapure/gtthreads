 ...
 * on error */
int uthread_create(uthread_pid u_pid, uthread_tid *tid, uthread_attr_t *attr,
...
{
	uthread_pid pid;
	uthread_tid tids[MAX_THREAD_COUNT];
	uthread_t *threads[MAX_THREAD_COUNT];
	int is_thread_done[MAX_THREAD_COUNT];
	int thread_count;
	int is_greedy;
}process;

process processes[2];
extern char sched_switch;
extern pid_t *k_tid;
extern int k_count;
